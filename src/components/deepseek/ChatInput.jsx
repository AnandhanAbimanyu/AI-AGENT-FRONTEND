...
export default function ChatInput({ onSend, disabled }) {
  ...
  const [type, setType] = useState('textToText');
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-4">
      <div className="flex gap-2 mb-4 flex-wrap">
        <button
          type="button"
          onClick={() => setType('textToText')}
          className={`px-3 py-1 rounded ${
            type === 'textToText' ? 'bg-blue-500 text-white' : 'bg-gray-200'
          }`}
        >
          Text
        </button>
        {/* <button
          type="button"
          onClick={() => setType('textToImage')}
          className={`px-3 py-1 rounded ${
            type === 'textToImage' ? 'bg-blue-500 text-white' : 'bg-gray-200'
          }`}
        >
          Text to Image
        </button>
